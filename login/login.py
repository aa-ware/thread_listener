...
class FormParser(HTMLParser):
    ...
    def convert_to_byte(self):
        for key, val in self.params.items():
            self.byteparams[key] = val.encode()
...
def auth(email, password, client_id, scope):
    ...
    def auth_user(email, password, client_id, scope, opener):
        ...
        parser.convert_to_byte()
        if parser.method == "POST":
            response = opener.open(parser.url, urllib.parse.urlencode(parser.byteparams).encode())
        else:
...
if __name__ == '__main__':
    ...
    client_id = "5489215" # Vk application ID
